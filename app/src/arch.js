...
export function drawArch(svg, art, opts = {}) {
  ...
  for (const it of items) {
    ...
    const [fill, stroke] = paint[st];
    P.push(`<path d="M${rx},${SY - 6} C${rx},${SY - 46} ${it.x0 + 16},${BY + BH + 20} ${it.x0 + 22},${BY + BH}" fill="none" stroke="${st === "off" ? GREY : stroke}" stroke-width="1.6"/>`);
    P.push(`<path d="M${it.x1 - 22},${BY + BH} C${wx - 16},${BY + BH + 20} ${wx},${SY - 46} ${wx},${SY - 12}" fill="none" stroke="${st === "off" ? GREY : stroke}" stroke-width="1.6"/>`);
    const title = it.kind === "attn" ? `attention ${it.bi + 1}` : `MLP ${it.bi + 1}`;
    const sub = it.kind === "attn" ? `${heads} heads · causal` : `${d}→${dff}→${d}`;
    P.push(chip(it.x0, BY, it.x1 - it.x0, BH, fill, stroke, title, sub, st !== "off"));
    if (st === "baked")
    ...
    P.push(plus(wx, SY, st === "off" ? "#aab3c4" : stroke));
  }
